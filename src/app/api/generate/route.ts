...
async function processTask(taskId: string, prompt: string, model: string, count: number) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { prompt, model = '@cf/stabilityai/stable-diffusion-xl-base-1.0', count = 1 } = body;

    console.log('Request:', JSON.stringify({ prompt: (prompt || '').substring(0, 50), model, count }));
